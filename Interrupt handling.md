...
- Schedule
    ...
    - Inserts interrupt in sorted order
    - [[Timer]] also calls schedule to create timer interrupts
- OneTick
    - Function that process one timer tick
    - Updates totalTicks which is a global variable
    - Invokes CheckIfDue to check if there is an interrupt that must be triggered
...
- CheckIfDue
    ...
    - If the timer interrupt for CPU is due, the Timer function Timer:TimerExpire will be called
- YieldOnReturn
